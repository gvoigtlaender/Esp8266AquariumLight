...
void handleSwitch() {
  ...
  if (server.args() > 0) {
    ...
    if (server.argName(0) == String("o")) {
      ...
      if (n >= CLight_1Ch::eAutomatic && n <= CLight_1Ch::eOff)
        m_pLight->SwitchControlMode((CLight_1Ch::E_LIGHTCONTROLMODE)n);
    }
    if (server.argName(0) == String("rst") && server.arg(0) == String("")) {
      ESP.restart();
    }
...
void loop(void) {
  ...
  case CButton::eLongClick:
    m_pLight->SwitchControlMode(CLight_1Ch::eAutomatic);
    m_pButton->setButtonState(CButton::eNone);
    break;
